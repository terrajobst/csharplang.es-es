...
```
`N1` contiene un miembro de tipo `A`, y `C` contiene un método estático `A`y porque `N2` importa ambos, que hacen referencia a `A` como un *simple_name* es ambiguo y un tiempo de compilación error. 
